...
year <- 2012
  ...
  #Fit gbm model
  N<-nrow(dx)
  indices.train<-sample(1:N,N*0.9)
  dx.train<-dx[indices.train,-c(index.IP,index.isSybill)]
  dx.test<-dx[-indices.train,-c(index.IP,index.isSybill)]
  ...
  prob<-predict(m2.gbm,dx.train,type="response")
  predictions<-ifelse(prob>cut.off,1,0)
  table(predictions, as.numeric(dx.train$BadExit)-1)
  # predictions     0     1
  # 0 15977  1924
  # 1 11561 25502

  prob<-predict(m2.gbm,dx.test,type="response")
  predictions<-ifelse(prob>cut.off,1,0)
  table(predictions, as.numeric(dx.test$BadExit)-1)
  # predictions    0    1
  # 0 1743  215
  # 1 1255 2895
